...
| `rayResourceAllocations`, `raySceneUploads`, `rayBLASUploads`, `rayInstanceUploads`, `rayMaterialUploads`, `rayOutputAllocations` | Lifetime allocations/upload events, including retired/failed backends. Output allocations count resize/reconfiguration transactions, not current textures. |
...
| `Test/RenderPerformanceInvariantTest.cpp`, `RunStandaloneTests.ps1`, `main.cpp`, `Fixtures/box.fbx` | Structural tests, full standalone recipe, real-GL gates and deterministic import fixture. |
