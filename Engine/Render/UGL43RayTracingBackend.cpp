...
bool CompileComputeProgram(GLuint& program, std::string& diagnostic, FRayTracingBackendStats& stats)
{
    ...
    const std::string assembledSource =
        SharedLightingShaderSource::BuildRayEffectsComputeShader();
    const char* source = assembledSource.c_str();
                                           ...
                                           std::string* diagnostic)
{
    ...
    if (mask & ShadowBit) makeTexture(candidateShadow, GL_RGBA16F, GL_RGBA);
                                         ...
                                         std::string* diagnostic)
{
    ...
    if (okay && uploadMaterials)
    {
        ...
        if (okay)
        {
            ...
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
            glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_SRGB8_ALPHA8, atlasWidth,
                                           ...
                                           std::string* diagnostic)
{
    ...
    if (!ResizeOutputs(inputs.width, inputs.height, mask,
        ...
        !UploadScene(packed, inputs.quality.anisotropy, diagnostic)) return false;
    ...
    glBindImageTexture(0, shadowedDirectTexture_, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA16F);
    ...
    glUniform1i(glGetUniformLocation(program_, "uDoReflections"),
                inputs.features.rayTracedReflections &&
                    inputs.quality.reflStrength > 0.0f ? 1 : 0);
    ...
    glUniform1i(glGetUniformLocation(program_, "uFrameIndex"),
                TemporalShaderFrameIndex({inputs.historySignature,
                                          inputs.frameIndex, false, 1}));
