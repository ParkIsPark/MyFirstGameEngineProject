...
Counters are lifetime cumulative, including backend switches and Shutdown/Init. Actual created/deleted GL names (`rayResourceAllocations`/`rayReleasedResources`) include shaders and rollback; `rayBufferUploadCalls`/`rayTextureUploadCalls` count submitted GL data/storage calls even when validation later rejects the transaction. `ray*UploadAttempts` and `rayOutputAllocationAttempts` count candidate transactions/categories. Existing `ray*Uploads` and `rayOutputAllocations` count successfully committed transactions only, not actual failed work. `resident*` and `live*` are current committed GPU ownership. Targets are caller-owned and report `OwnedAttachmentCount()` separately. No normal frame may increment CPU bridge counters. Observe work at the call site, not from Actor counts; do not add readbacks for telemetry. Run `Test/RunStandaloneRunnerSelfTest.ps1` to exercise runner exception/timeout cleanup as well as the normal standalone matrix.
