...
### Lua Actor scripting (`Engine/Script`)

`UScriptSubsystem` is the sole Lua 5.4.9 VM owner and shuts down after world `EndPlay`. `FLuaBindingRegistry` is the future extension seam, but the only public Lua function today is `Engine.Log(string)`. Do not add Actor, transform, World, Input, renderer, physics, spawning, reflection, or host-library bindings as incidental fixes. The safe library set is base/coroutine/table/string/math/utf8; `io`, `os`, `package`, `debug`, `dofile`, `loadfile`, and per-instance `load` stay unavailable.

`FLuaScriptCache` validates and compiles normalized project-relative paths under `Content/Scripts`; it rejects traversal and physical reparse/symlink escape. Compiled bytecode is shared, while every `UScriptComponent` owns a separate `FLuaScriptInstance` environment and callback registry references. `BeginPlay`, `Tick(deltaSeconds)`, and `EndPlay` are optional and run only in PIE/standalone. There is no hot reload: `UWorld::BeginPlay` clears the session cache so edits apply on the next Play.

Failure policy is component-local: load/syntax/Begin failures disable that instance; a Tick failure reports once and suppresses later Ticks without changing serialized `Enabled`, but retains a successfully begun instance so Stop invokes its `EndPlay` exactly once before releasing it; End failure reports once but cannot stop later cleanup. Attachment diagnostics must retain the labeled `Actor`, exact `ScriptComponent[index]`, normalized `Path`, `Phase`, Lua message, and Lua file/line when available. Never let a Lua/native diagnostic sink escape a lifecycle boundary. Stop order is `UWorld::EndPlay → AActor::DispatchEndPlay → UScriptComponent::EndPlay → FLuaScriptInstance::EndPlay/registry unrefs`; subsystem shutdown invalidates any outstanding instance handles before `lua_close`.

World format 2 stores only `Type = ScriptComponent`, `Enabled`, and an optional `Script = Content/Scripts/...` line; an unassigned component omits `Script`. VM state, closures, environments, callbacks, and mutable Lua values are runtime-only. The editor may add multiple attachments and assign/import only `.lua` assets; packaging includes every safe `.lua` beneath `Content/Scripts`, referenced or not. The cube rotation remains authored C++ `AActor::Tick`, because this Lua milestone cannot manipulate transforms and does not provide click-to-place. See `docs/scripting/lua-actor-scripting.md` for the complete call stacks, schema, commands, and boundaries.
